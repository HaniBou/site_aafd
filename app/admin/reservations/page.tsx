              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                  {reservations.length > 0 && (
                    <button
                      onClick={exporterCSV}
                      className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-medium px-4 py-2 rounded-lg transition-colors text-sm"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:h-5 md:w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm3.293-7.707a1 1 0 011.414 0L9 10.586V3a1 1 0 112 0v7.586l1.293-1.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clipRule="evenodd" />
                      </svg>
                      <span className="whitespace-nowrap">Télécharger CSV</span>
                    </button>
                  )}
                  <div className="bg-indigo-50 px-4 py-2 rounded-lg text-center sm:text-left">
                    <div className="text-xs text-indigo-600 font-medium">Total commandé</div>
                    <div className="text-xl md:text-2xl font-bold text-indigo-600">{totalGeneral}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-4 md:space-y-6">
          {reservations.length === 0 ? (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 md:p-8 text-center">
              ...
              <p className="text-xs md:text-sm text-gray-400 mt-2">Les réservations apparaîtront ici</p>
            </div>
          ) : (
            Object.entries(reservationsParPlat).map(([platNom, reservationsPlat]) => {
              const totalPlat = calculerTotalPlat(reservationsPlat);
              return (
                <div key={platNom} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                  {/* En-tête du plat */}
                  <div className="bg-gradient-to-r from-orange-50 to-orange-100 border-b border-orange-200 p-3 md:p-4">
                    <div className="flex justify-between items-center gap-2">
                      <h2 className="text-base md:text-lg font-bold text-gray-900 truncate">{platNom}</h2>
                      <div className="bg-orange-600 text-white px-2 md:px-3 py-1 rounded-lg shrink-0">
                        <span className="text-xs md:text-sm font-semibold">Total: {totalPlat}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="divide-y divide-gray-200">
                    {reservationsPlat.map((reservation) => (
                      <div key={reservation.id} className="p-3 md:p-4 hover:bg-gray-50 transition-colors">
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          <div className="bg-orange-50 px-2 py-0.5 rounded">
                            <span className="text-xs font-semibold text-orange-600">x {reservation.quantite}</span>
                          </div>
                          ...
                          {reservation.emailEnvoye === false && (
                            <div className="flex items-center gap-1 bg-yellow-100 px-2 py-0.5 rounded" title={reservation.emailErreur || "Email non envoyé"}>
                              ...
                              <span className="text-xs font-semibold text-yellow-700 hidden sm:inline">Email non envoyé</span>
                            </div>
                          )}
                          {reservation.emailEnvoye === true && (
                            <div className="flex items-center gap-1 bg-green-100 px-2 py-0.5 rounded">
                              ...
                              <span className="text-xs font-semibold text-green-700 hidden sm:inline">Email OK</span>
                            </div>
                          )}
                        </div>

                        <div className="flex flex-col sm:flex-row sm:items-start gap-3 mb-3">
                          <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div>
                              <div className="text-xs text-gray-500 mb-0.5">Client</div>
                              <div className="text-sm font-medium text-gray-900 truncate">{reservation.clientNom}</div>
                            </div>
                            <div>
                          ...
                          <button
                            ...
                            className="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-white font-medium px-3 py-2 sm:py-1.5 rounded text-xs transition-colors shrink-0"
                          >
                            Annuler
                          </button>
                        </div>
                        ...
                        {reservation.message && (
                          <div className="p-2 bg-blue-50 rounded border border-blue-200">
                            ...
                            <div className="text-xs md:text-sm text-gray-700">{reservation.message}</div>
                          </div>
                        )}
                      </div>
                    ))}
