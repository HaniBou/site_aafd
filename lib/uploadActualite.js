 ...
 * @param {string} [actualite.date] - La date de l'actualité (optionnel, utilise la date du jour si absent)
...
export default async function uploadActualite(actualite) {
  try {
    ...
    const actualiteWithDate = {
      ...actualite,
      slug,
      date: actualite.date || new Date().toISOString(), // Utiliser la date fournie ou la date actuelle
